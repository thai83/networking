...
/* Helper functions...*/
unsigned short ntohs_2 (unsigned char bytes[], int offset) {   /* Convert a short int from network order to little endian order. */
...
void decodeArpReply (unsigned char *msg) { /* Decode ARP response packets. */

    int index=14, count;
    unsigned short temp;

    temp = ntohs_2(msg, index); index += 2;
    ...
    printf ("Sender hardware address: "); for (count=1; count <=MAC_ADDR_LEN; count++, index++) printf ("%0x ", msg[index]); printf("\n");

    printf ("Sender Protocol Address: "); for (count=1; count <=IP_ADDR_LEN; count++, index++) printf ("%d ", msg[index]);  printf("\n");

    printf ("Target hardware address: "); for (count=1; count <=MAC_ADDR_LEN; count++, index++) printf ("%0x ", msg[index]); printf("\n");

    printf ("Target Protocol Address: "); for (count=1; count <=IP_ADDR_LEN; count++, index++) printf ("%d ", msg[index]);  printf("\n");
...
int main(int argc, char** argv) {
    ...
    unsigned char *dest_ip; struct in_addr ip_in;
    ...
    if (inet_aton(argv[1], &ip_in) == 0) { /* Check for a valid IP address and convert it into binary data. */
    ...
    fd = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ARP));   /* (or 0x0806): Socket accepts ARP packets only. */
    ...
    */
    ...
    if (ioctl (fd, SIOCGIFCONF, &ifc) == -1) return 1;
    ...
    for (index=0; index < array_size; index++) {
        ...
        if (argc > 2 && strcmp (ifr.ifr_name, argv[2]) != 0) continue;

        memcpy (src_intfaddr.ip_addr, ifr.ifr_addr.sa_data + 2, IP_ADDR_LEN); /* Get IP address of this interface. */

        if (ioctl (fd, SIOCGIFFLAGS, &ifr) == -1) return 1;   /* Get device interface flags. */
        if (ifr.ifr_flags & IFF_LOOPBACK) continue;  /* Ignore loopback interface. */
        if (!(ifr.ifr_flags & (IFF_RUNNING | IFF_UP | IFF_DYNAMIC))) {
            printf ("Interface %s is not active...\n", ifr.ifr_name);
            continue;
        }
        /* Only support Ethernet devices for now. */
        if (strncmp (ifr.ifr_name, "eth", 3) != 0 && strncmp (ifr.ifr_name, "wlan", 4) != 0) continue;

        if (ioctl(fd, SIOCGIFHWADDR, &ifr) == -1) return 1;   /* Get hardware information of this interface. */
    ...
    if (arpr.arp_flags & ATF_COM) {
        ...
        for (index=0; index < MAC_ADDR_LEN; index++) printf ("%0x ", (unsigned char)arpr.arp_ha.sa_data[index]); printf("\n");
    ...
    retval = sendto(fd, send_buf, sizeof(send_buf), 0, NULL, 0);
    ...
    retval = recvfrom(fd, recv_buf, sizeof(recv_buf), 0, NULL, 0);
    ...
    for (index=0; index<sizeof(recv_buf); index++) printf ("%0x", recv_buf[index]);
    ...
    if (ioctl (fd, SIOCSARP, &arpr) == -1) return 1; /* Add to MAC table */
    
    /* Print out some information useful for debugging. */
    if (arpr.arp_flags & ATF_PUBL) printf ("\nPublish Entry...\n");
    for (index=2; index < MAC_ADDR_LEN; index++) printf ("%d ", (unsigned char)arpr.arp_pa.sa_data[index]); printf("\n");
    for (index=0; index < MAC_ADDR_LEN; index++) printf ("%0x ", (unsigned char)arpr.arp_ha.sa_data[index]); printf("\n");
